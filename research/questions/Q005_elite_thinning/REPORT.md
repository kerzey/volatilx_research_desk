...
### Post-sign-off addendum (2026-09-11)

Checklist item 10 (a second-opinion review by an independent model) was skipped at sign-off (8242ccf) because of a credential rule that has since been resolved; the red team ran it afterward and logged the result as an addendum to `results/REDTEAM.md`, dated 2026-09-11, above the final sign-off line. **This does not change the verdict; INCONCLUSIVE stands, and the controller remains LEDGERED.**

- **a. The second opinion, run late, agrees.** `scripts/second_opinion.py` was run on 2026-09-11 against model **gpt-5.6-sol**, after sign-off, once the credential it needs (`OPENAI_API_KEY`) cleared CLAUDE.md's allowlist. Shown only `PREREG.md`, `eval.py`, and `SUMMARY.md` (not this review), the second model independently concluded the configuration-causation claim is unsupported and closed with "SECOND OPINION: DISAGREE" — aimed at eval.py's mechanical HISTORICALLY_CONFIRMED, not at the controller-recorded verdict, which was already INCONCLUSIVE. The red team's full point-by-point response to all eight of its critique sections is in `results/REDTEAM.md`'s addendum; the net effect recorded there is "Verdict unchanged: INCONCLUSIVE. No change is proposed to the controller, LEDGER.md or REPORT.md."

- **b. A new data-quality finding (A1): four nights' stored candidates come from a late re-run, not the 16:05 ET output.** One of the second model's look-ahead points led the red team to check row-creation timestamps. On **2026-04-02, 04-24, 05-15, and 07-02**, the whole candidate set on file was written by a re-run 19–51 hours after the pick night (e.g., the 07-02 run row was created at 22:23 UTC that night, but every candidate row carries a re-run timestamp of 07-03 14:58 UTC) rather than the 16:05 ET decision-time output. Consequence for this question: **two of the four dates eval.py flagged as "config changes bracketing the fall" — 07-02 and its revert 07-07 — actually describe how that one re-run happened to be configured, not a change to the engine's 16:05 configuration.** This strengthens, not weakens, the red team's override of eval.py's mechanical HISTORICALLY_CONFIRMED-configuration verdict. Gate 0 (the fall itself) survives dropping all four nights: recomputed on the remaining 103 nights, P0 = 28/55 = 0.509/night vs P1 = 11/48 = 0.229/night, trend p = 0.035 (was 0.039), permutation rate p = 0.010 (was 0.018), exact rate p = 0.024 (was 0.041) — all p-values reported in the REDTEAM.md addendum, not recomputed here. This has been referred to the Data Steward for a freeze-wide audit of re-run nights across `manifest_v001` (report in progress: `research/reports/KT_AUDIT_manifest_v001_rerun_nights.md`); 2026-05-15's weight/ATR configuration date may likewise really date from the 05-17 re-run, though that test did not fire either way (p=0.062).

- **c. Post-selection caveat, sharpened.** This question was posed because the monthly decline in elite-candidate counts had already been noticed before the PREREG was written. Since every elite candidate was published in both periods (29/29, 12/12), Gate 0's test is run on essentially the same series that motivated the question — a post-selection p-value, which overstates the evidence beyond what a pre-specified, blind test would. This does **not** move the verdict to NULL: PREREG §8 only returns NULL if Gate 0 fails, and the registered test passes as run. It sharpens the existing caveat: the fall is real, but read it as statistically marginal, not as a clean, independently-confirmed signal.

- **d. Tooling note (not verdict-relevant).** `scripts/second_opinion.py` opens the input files at line 39 and writes `SECOND_OPINION.md` at line 49 without `encoding="utf-8"`. On the Windows host that ran it, UTF-8 source text was decoded as cp1252, so characters such as ≥, §, →, and — reached the model garbled (and were saved garbled in the output file). The garbling did not stop the model from reading the code or reaching a verdict; REDTEAM.md's addendum de-gargles the quoted critique for the record. This is a fix for Haci to make outside the desk's write scope (`scripts/` is not under `research/`, `docs/`, or `playbook/`).
