...
| EN-020 | plumbing | READY | — | — | **Intraday UOA sampler: option buy/sell side from the quote at trade time (fixes PI-023).** A 10-minute WebJob, 09:40–16:20 ET, takes a chain snapshot per universe underlying (±12% strikes, ≤ 180 DTE), keeps the latest quote per contract, and for contracts whose day volume rose since the last tick stores the two quotes that bracket the interval, fetches that interval's prints with a two-tick lag (clears Alpaca Basic's 15-minute hold-back) and classifies each print against the bracketing quotes (same 0.10-spread edge as today; conflict → unknown). Buckets go to `uoa_contract_intraday`; the nightly sums them when coverage ≥ 0.90 and labels every row `aggressor_source` (`quote_at_trade` / `closing_quote` / `none`), so `dir_ratio`, the SAS flow vote, Whale Watch and the UOA conviction column improve on the ship date without further change. Omega's `get_option_trades` reads side columns from the buckets (never inferred against the current quote); an admin-only "UOA now" view shows live buy-side premium by underlying. ~700–1,100 requests a tick, inside Basic's 200/min. Phase 2 (desk decides after 20 sessions): delta-adjusted closing-quote backfill for history, calibrated against Phase 1 labels. No history rewritten; ship date to DATA_NOTES, Q014/Q019/Q029/Q030 split (DP-50). Design, budget, tests and the steward's W1–W6: `research/reports/uoa_side_2026-09-15/REPORT.md` §5. Red-team reviewed 2026-09-15 (§7 of that report). Source: Haci 2026-09-15 ("fetch UOA live every 10 minutes and aggregate to the day"). |
