...
echo "PROD_SAS_TOKEN (read+list only, expires $EXPIRY):"
...
echo
echo "RESEARCH_SAS_TOKEN (read/write/list/create, expires $EXPIRY):"
...
echo "  az storage blob upload --account-name $ACCOUNT -c $PROD_CONTAINER -n _probe.txt -f /dev/null --sas-token \"\$PROD_SAS_TOKEN\""
